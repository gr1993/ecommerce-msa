...
@Schema(description = "관리자 쿠폰 상세 응답")
@Getter
@Builder
public class AdminCouponDetailResponse {
    ...
    @Schema(description = "쿠폰 발급 내역")
    private List<AdminCouponUsageResponse> userCoupons;

    public static AdminCouponDetailResponse from(Coupon coupon) {
        List<AdminCouponUsageResponse> usages = coupon.getUserCoupons().stream()
                .map(AdminCouponUsageResponse::from)
                .toList();
