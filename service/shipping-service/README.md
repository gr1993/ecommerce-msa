...
하나의 주문에 여러 상품이 포함되어 있고 교환 신청한 (수량이 0이 아닌) 상품 목록만 교환이 진행된다.  
그래서 실제로 사용자가 택배 박스에 교환한다고 한 품목만 담겨 있다고 가정할 것이다.
옵션 변경 교환의 경우 옵션별로 가격이 다를 수 있으나 추가 결제로직은 빼고 재고 수량 조절만 진행될 예정이다.  
