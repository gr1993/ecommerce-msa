...
이번 프로젝트를 통해 MSA 환경에서 Kafka를 활용하여 EDA(Event-Driven Architecture) 방식으로 서비스 간 통신을 구현하는  
방법을 깊이 이해할 수 있었다. 또한 시스템을 구현하는 과정에서 발생한 다양한 문제들을 고민하며 DLQ(Dead Letter Queue),  
Transactional Outbox Pattern, Saga Pattern(보상 트랜잭션) 등을 직접 도입해 보면서 관련 개념을 실무적인 관점에서 익힐  
수 있었던 점이 큰 도움이 되었다.  

프로젝트를 마무리한 뒤 전체 구조를 다시 복기하면서 한 가지 중요한 부분을 놓치고 있었다는 것을 깨달았다. EDA 기반 시스템에서는  
특정 서비스에서 상태 변화가 발생했을 때 해당 서비스가 자신의 상태 변화(Source of Truth)를 이벤트를 통해 명확하게 외부에 전파  
해야 한다는 점이다.  

주문 서비스와 결제 서비스 간 통신을 구현할 때 주문 취소가 발생하면 OrderCancelled 이벤트를 발행하도록 설계했다. 하지만 이를  
구독하는 결제 서비스에서는 실제 결제 취소 처리를 수행한 뒤, 이벤트의 무한 반복을 방지하기 위해 PaymentCancelled 이벤트를  
별도로 발행하지 않았다.  

프로젝트를 복기하며 생각해 보니 이러한 방식보다는 결제 서비스에서도 결제 취소가 완료된 시점에 PaymentCancelled 이벤트를 명확하게  
발행하는 것이 더 적절한 설계라고 판단했다. 그리고 주문 서비스에서 이를 다시 구독할 때 무한 이벤트 루프를 단순히 우려하기보다는  
주문 서비스의 PaymentCancelled 구독 로직에 멱등성을 잘 고려하거나, PaymentFailed와 같은 별도의 이벤트를 정의하여 상황에 따라 
구독하도록 설계하는 것이 이벤트의 책임과 역할을 명확하게 분리하는 데 더 도움이 된다는 점을 깨달았다.  

이번 경험을 통해 EDA 환경에서는 단순히 이벤트를 전달하는 것에 그치지 않고, 각 서비스가 자신의 상태 변화를 어떤 이벤트로 표현하고  
어떤 책임을 가지는지 명확히 설계하는 것이 매우 중요하다는 점을 배울 수 있었다.  